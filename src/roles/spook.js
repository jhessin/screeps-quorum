...
class Spook extends MetaRole {
  ...
  signController(creep) {
    ...
    if (
      ...
      creep.room.controller.reservation.username !== USERNAME
    ) {
      return false;
    }
    // Not ours (owned)
    if (
      ...
      creep.room.controller.owner.username !== USERNAME
    ) {
      return false;
    }
    // Signed too recently
    if (
      creep.room.controller.sign &&
      Game.time - creep.room.controller.sign.time > CONTROLLER_RESIGN_COOLDOWN
    ) {
      return false;
    }
